...
int main(int argc, char** argv)
{
    ...
    double* ce = (double*) malloc(sizeof(double) * m);
    assert(c);
    assert(ce);
    for(int i = 0; i < m; i++) {
        c[i] = b[i] - bb[i];
    ...
    double norme = dnrm2(m, ce, 1);
    printf("2-Norm between CPU and GPU-CSR answers: %e\n", norm);
    printf("2-Norm between CPU and GPU-ELL answers: %e\n", norme);
                        ...
                        int* n_new)
{
  ...
  *n_new = 0;
  ...
  for (i = 0 ; i < m+1 ; i++)
    {
      diff = csr_row_ptr[i] - csr_row_ptr[i-1];
      if (diff > *n_new)
	*n_new = diff;
    }

  int length_ell = (*n_new) * m;
  unsigned int* ell_col_ind_ = (unsigned int*) malloc(sizeof(unsigned int) * length_ell);
  double* ell_vals_ = (double*) malloc(sizeof(double) * length_ell);
  ...
  for (i = 0 ; i < m ; i++)
    {
      begin_row = csr_row_ptr[i];
      end_row = csr_row_ptr[i+1];
      length = end_row - begin_row;

      for (int j = 0 ; j < (*n_new) ; j++)
	{ 
	  if (j < length)
	    {
	      ell_col_ind_[count] = csr_col_ind[begin_row];
	      ell_vals_[count] = csr_vals[begin_row];
	      begin_row++;	 
	    }
	  else
	    {
	      ell_col_ind_[count] = 0;
	      ell_vals_[count] = 0;
	    }
	  count++;
	}
  ...
  *ell_col_ind = ell_col_ind_;
  *ell_vals = ell_vals_;

}

void printell(int m, unsigned int* ell_col_ind, double* ell_vals, 
	       int n_new)
{
  ...
  for (i = 0 ; i < m*n_new ; i++)
      printf("\t%d", ell_col_ind[i]);  
  printf("\n\n");
  for (i = 0 ; i < m*n_new ; i++)
      printf("\t%.2f", ell_vals[i]);
